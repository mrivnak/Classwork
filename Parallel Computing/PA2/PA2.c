...
int main(int argc, char** argv){
     ...
     *      Rank 0 SEND the data and everyone else RECEIVES the data
     ...
     */
    int myN = ((int) (N / numranks)) + ((N % numranks > rank) ? 1 : 0);

    /*
     * Allocates counts and displacements for all ranks
     */
    int * sendcounts = (int *) malloc(myN * sizeof(int));
    int * disp = (int *) malloc(myN * sizeof(int));

    /*
     * Compute Counts
     */
    for(int i=0; i<numranks; i++){
        sendcounts[i]=((int) (N / numranks)) + ((N % numranks > i) ? 1 : 0);;
    }
     ...
     *        everyother rank. Every other rank will receive the scattered data
    ...
    /*
     * Do dot product
     * PLEASE NOTE HERE.  The computation of this partial dot
        oldDisp += newDisp; product is the same.
    ...
    for(int i=0;i<sendcounts[rank];i++){
        // printf("Rank %d accessing index %d\n", rank, i);
        result+=mya[i]*myb[i];
    }
    ...
    /*
    // TODO: 7   
        use MPI_Reduce or MPI_Allreduce, lets use MPI_Allreduce so that the 
